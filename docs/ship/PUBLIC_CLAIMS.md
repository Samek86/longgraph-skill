...
| P3 | Executor write-set destinations resolve inside the workspace and cannot clobber `run_dir` scoreboard files via relative escape. | `test_write_set_cannot_escape_workspace` `test_executor_cannot_clobber_ledger_via_relpath` |
...
| P6 | Missing / incomplete `blocked-on` findings are scout-only. Live Current-slice `owner_blocked` skips write-set and close. | `test_blocked_on_skips_executor_until_findings` `test_owner_blocked_skips_write_set_and_close` |
