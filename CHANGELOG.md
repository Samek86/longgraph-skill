...
- M-TIP-2: ACCEPT-GATE / watermark fold runs after any applied
  write-set (not MockHost-only). Pre-apply same-tick next-milestone
  release stays on the applied-work Host path. Emit/timer hosts
  still do not apply.
